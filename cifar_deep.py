...
class deepnn(Network):
  ...
  def forward(self, images, labels):
    ...
    self.ortho_loss += self._ortho_loss(W_conv, b_conv)
    h = self._nonlin(self._conv2d(images, W_conv, b_conv))
    ...
    with tf.variable_scope('conv4'):
      W_conv, b_conv = self._conv_weights_fn([3, 3, 64, 64])
    self.ortho_loss += self._ortho_loss(W_conv, b_conv)
    h = self._nonlin(self._conv2d(h, W_conv, b_conv, padding='VALID'))
    ...
    self.cross_entropy = tf.reduce_mean(cross_entropy_vector)
...
def main():
  ...
  parser.add_argument('--ortho-weight', default=0, type=float, 
                      help='weight given to ortho loss (default %(default)s)')
  ...
  logits = net.forward(images, labels)
  cross_entropy = net.cross_entropy
  accuracy = net.accuracy
  ortho_loss = net.ortho_loss
  wtd_ortho_loss = ortho_loss*options.ortho_weight
  total_loss = cross_entropy + wtd_ortho_loss
  ...
  for i in range(options.num_epochs):
    for j in range(0, len(x_train), 128):
      iters = iters + 1
      _, train_cross_entropy, train_accuracy, train_ortho_loss, train_wtd_ortho_loss, train_total_loss = sess.run([train_step, cross_entropy, accuracy, 
                                                                                                                   ortho_loss, wtd_ortho_loss, total_loss], 
      ...
      train_metrics['ortho_loss'].append(train_ortho_loss)
      train_metrics['wtd_ortho_loss'].append(train_wtd_ortho_loss)
      ...
      if (iters-1) % 100 == 0:
        ...
        print('step %d, train accuracy %g, train cross entropy %g, train ortho loss %g, train weighted ortho loss %g, train total loss %g' % (iters, 
                                                                                                                                              train_accuracy, 
                                                                                                                                              train_cross_entropy, 
                                                                                                                                              train_ortho_loss, 
                                                                                                                                              train_wtd_ortho_loss,
                                                                                                                                              train_total_loss
                                                                                                                                          ))
