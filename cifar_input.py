...
def _download_dataset(dataset):
  ...
  file_path, _ = six_urllib.request.urlretrieve(dataset_url, file_path, _progress)
  statinfo = os.stat(file_path)
  print('Successfully downloaded', file_name, statinfo.st_size, 'bytes.')

  data_path = os.path.join('data', dataset)
  tarfile.open(file_path, 'r:gz').extractall(tmp_dir)

  data_folders = glob.glob(os.path.join(tmp_dir, '*/'))
  assert len(data_folders)==1
  shutil.move(data_folders[0], data_path)
...
def build_input(dataset, batch_size, mode):
  ...
  bin_path = os.path.join(data_path, 'test*.bin')
  data_files = tf.gfile.Glob(os.path.join(bin_path))
  if(mode=='train'):
    bin_path = os.path.join(data_path, '*.bin')
    data_files = list(set(tf.gfile.Glob(bin_path)) - set(data_files))
